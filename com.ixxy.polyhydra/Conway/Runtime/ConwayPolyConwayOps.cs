...
//						    //newVertices[edge.Prev.PairedName],
        ...
        public ConwayPoly LoftAlongProfile(OpParams o, AnimationCurve profile,
            ...
            bool rotateProfile = false, bool lace=false)
        {
            var ratioValues = profile.keys.Select(x => x.value).ToList();
            var offsetValues = profile.keys.Select(x => x.time).ToList();
            ...
            return LoftAlongProfile(o, 
                ...
                rotateProfile: rotateProfile, lace: lace);
        }
            ...
            for (var faceIndex = 0; faceIndex < Faces.Count; faceIndex++)
            {
                ...
                if (IncludeFace(faceIndex, o.facesel, o.GetTagList(), o.filterFunc))
                {
                    ...
                    for (int section = 0; section < offsetValues.Count; section++)
                    {
                        ...
                        if (rotateProfile)
                        {
                            profileX = offsetValues[section];
                            profileY = 1 - ratioValues[section];
                        }
                        else
                        {
                            profileX = offsetValues[section];
                            profileY = ratioValues[section];
                        }
