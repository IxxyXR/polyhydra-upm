Shader "Unlit/PrettyCrystalTris"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float3 baseColor = tex2D(_RampTex, abs((v.uv*.5)-1));
                ...
                float4 edges = (pow(map(v.uv2.y, _Edge1, _Edge2, 0, 1), _Falloff1) * _Falloff2);
                return float4(col.xyz * edges.xyz, 1);//saturate(float4(col,3*length(col) ));
            }
