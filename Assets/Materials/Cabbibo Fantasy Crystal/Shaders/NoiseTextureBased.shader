...
Shader "FantasyCrystals/NoiseTextureBased"
{
...
float totalSmoke = 0;
  float3 rd = v.rd;
  float3 ro = v.ro + tri3( v.ro * _NoiseSize ) * _NoisePower;
  ...
  float4 PP = projectOnPlane( planePos, planeUp , ro , -v.rd );
  float3 fPos = PP.xyz;
 ...
 float2 xy = float2(x+sin(float(i) * 5121.), y+sin(float(i) * 4421.));
  float3 newCol = 10*tex2D(_SampleTexture, xy * _SampleSize)* saturate( length(fPos - ro)) * match * saturate((1/(20*abs(PP.w)*abs(PP.w))));//abs(dot( normalize(planeUp), normalize(v.rd)));
